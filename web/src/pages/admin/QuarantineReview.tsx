...
export function QuarantineReview() {
    ...
    const columns = [
        ...
        columnHelper.accessor('status', {
            ...
            cell: info => {
                ...
                if (s === 'approved') return <span className="px-2 py-0.5 rounded text-xs font-medium border text-emerald-400 bg-status-success/10 border-status-success/20">Approuvé (Reprocessé)</span>
                return <span className="px-2 py-0.5 rounded text-xs font-medium border text-red-600 dark:text-red-400 bg-red-500/10 bg-red-500/10 border-status-error/20">Rejeté</span>
            },
        }),
        columnHelper.display({
            id: 'actions',
            ...
            cell: info => {
                ...
                return (
                    <div className="flex items-center gap-2">
                        <Button variant="ghost" className="p-1.5 h-auto text-brand-400 hover:bg-[#137fec]/10" onClick={() => setViewingItem(item)} title="Examiner">
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="bg-surface-800/80 backdrop-blur-md border border-surface-700 p-4 rounded-xl flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            >
                {viewingItem && (
                    <div className="space-y-6">
                        <div className="bg-red-500/10 bg-red-500/10 border border-status-error/30 p-4 rounded-lg flex items-start gap-3">
                        ...
                        {viewingItem.status !== 'pending' && (
                            <div className="text-sm border-t border-surface-700 pt-4 mt-4">
                        ...
                        {viewingItem.status === 'pending' && (
                            <div className="border-t border-surface-700 pt-6">
                                ...
                                <textarea
                                    className="w-full bg-surface-900 border border-surface-700 rounded-lg px-3 py-2 text-text-primary focus:ring-2 focus:ring-brand-500 min-h-[80px] mb-4"
