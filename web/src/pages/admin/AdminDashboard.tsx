...
export function AdminDashboard() {
    ...
    const StatCard = ({ title, value, icon, to, description }: { title: string, value: string | number, icon: React.ReactNode, to: string, description: string }) => (
        <Link to={to} className="block group">
            <div className="bg-surface-800/80 backdrop-blur-md border border-surface-700 rounded-xl p-6 h-full transition-all duration-200 hover:border-brand-500/50 hover:bg-surface-700/50">
                <div className="flex items-start justify-between mb-4">
                    <div className="p-3 bg-surface-700/50 rounded-xl text-brand-400 group-hover:scale-110 group-hover:bg-[#137fec]/10 group-hover:text-brand-300 transition-all">
                        {icon}
                    </div>
                </div>
                <div>
